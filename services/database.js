...
class Database {
  ...
  /**
   * Задача 8: Обновление системного промпта бота
   */
  async updateBotPrompt(telegramUsername, systemPrompt) {
    const normalized = telegramUsername.trim().toLowerCase();
    ...
    const res = await this.query(sql, [systemPrompt, username]);
    
    if (res.length === 0) {
      throw new Error(`Бот ${username} не найден`);
    }
    
    log(`✅ Промпт обновлен для ${username}`);
    return true;
  }
}
